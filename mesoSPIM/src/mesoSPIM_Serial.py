...
from .mesoSPIM_Stages import mesoSPIM_PI_1toN, mesoSPIM_PI_NtoN, mesoSPIM_PI_1toNwithCuvette, mesoSPIM_ASI_Stages, mesoSPIM_DemoStage, mesoSPIM_PI_rotz_and_Galil_xyf_Stages
...
class mesoSPIM_Serial(QtCore.QObject):
    ...
    def __init__(self, parent):
        ...
        elif self.cfg.stage_parameters['stage_type'] == 'PI_1controllersNstagesWithCuvette':
            self.stage = mesoSPIM_PI_1toNwithCuvette(self)
...
#        self.sig_pause.emit(boolean)
    ...
    def set_zoom(self, zoom, wait_until_done=False):
        ...
        ''' Here, the state parameters are set before sending the value to the zoom --
        this is to avoid laggy update loops with the GUI.'''
        if self.state['zoom'] == zoom or self.state['state'] == 'init':
            self.state['zoom'] = zoom
            try:
                self.state['pixelsize'] = self.cfg.pixelsize[zoom]
            except:
                errorstring = 'Pixelsize for Zoom ' + str(zoom) + 'not in config file!'
                logger.error(errorstring)

            setup_in_live_mode = False
            if self.cfg.cuvette_control:
                new_cuvette_pos = self.state['extra_info']['zoom_cuvette_dict']['Safe cuvette pos']
                logger.info('Setting cuvette position to (safe pos): ' + str(new_cuvette_pos))
                self.move_absolute({'c_abs': new_cuvette_pos}, wait_until_done = True)
                self.send_status_message('Moving cuvette to safe position')
            
            # check that we are not in live mode: turret usually takes so long that bleaching should be avoided
            ...
            if self.cfg.cuvette_control:
                ...
                self.move_absolute({'c_abs': new_cuvette_pos}, wait_until_done = True)
                self.send_status_message('Moving cuvette to objective-specific position')

            # print('Current state: ', str(self.state['state']))
            if self.state['state'] == 'idle' or self.state['state'] == 'live':
                # print('mesoSPIM Serial: Finished fired')
                self.sig_finished.emit()

            ''' Restart live mode afterwards?
            if self.cfg.zoom_parameters['zoom_type'] == 'PI_Turret':
            ...
            logger.info('Zoom already set to the correct value')
            self.send_status_message('Zoom already set to the correct value')
